...
def build_m2_review(out, cfg=None):
    ...
    family_strength = review[FAMILY_STRENGTH_COLS].apply(pd.to_numeric, errors="coerce")
    review["family_consensus_score"] = family_strength.mean(axis=1, skipna=True)
    review["evidence_family_count"] = review[FAMILY_RANK_COLS].apply(pd.to_numeric, errors="coerce").notna().sum(axis=1)
    review["family_rra_p"] = review.apply(lambda row: _rra_p_value(row, FAMILY_RANK_COLS, n_sessions), axis=1)
