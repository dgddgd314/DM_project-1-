...
def normalize_live_feature(src, dst, cfg):
    ...
    if live_window:
        windows = list(cfg.setdefault("time", {}).get("valid_windows", []))
        if live_window not in windows:
            windows.append(live_window)
            cfg["time"]["valid_windows"] = windows
            print(f"[INFO] temp cfg valid_windows appended for live run: {live_window}")
...
def make_temp_inputs(run_id, live_feature):
    ...
    normalize_live_feature(live_feature, feat_dir / live_feature.name, cfg)

    temp_cfg = work / "cfg.yml"
    with open(temp_cfg, "w", encoding="utf-8") as f:
        yaml.safe_dump(cfg, f, allow_unicode=True, sort_keys=False)
    return work, temp_cfg, out_dir
...
def main():
    ...
    work, temp_cfg, out_dir = make_temp_inputs(run_id, feature)
